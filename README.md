...
```
The `edges` and `rec` parameters can be taken from a `cmsdf_decompose_result`.
`pixels` is a buffer that will be filled with BGRA pixels (8-bit per component) encoding the MSDF.
The caller needs to ensure sufficient capacity to store an image with dimensions specified by `rec`.
Passing `NULL` for pixels immediately returns the minimal buffer size required in bytes.
Otherwise the number of bytes written is returned.

`cmsdf_draw_edges(cmsdf_edge_array edges, cmsdf_rec rec, uint32_t* pixels)` works the same, except that outputs an images showing the internal edge color channel assignment.
...
```
`pixels` needs to be large enough to accommodate `width * height` elements.
